...
noise_curve_data = np.loadtxt('NoiseCurve_Groundkm.dat').T
...
def get_htilde(
   ...
   time_vec_heliocentric, Fp, Fc = antennaFuns_ground.time_delay_antenna_funs(
      time_vec_detector, 
      source_RA, 
      source_DEC, 
      source_psi, 
   ...
   hp, hc = waveform_LO.get_hp_hc(
      source_Mc, 
      source_iota, 
      source_dL, 
      source_phi0, 
      source_tc-time_vec_heliocentric
      )
...
class GW_event:
   def __init__(
      ...
      Detector inputs:
         detector_fGWmin -- smallest frequency considered [Hz]
         detector_fGWmax -- smallest frequency considered [Hz]
         detector_t0 -- reference time at which to define the location of the
                        detector [seconds after vernal equinox]
         detector_RA0 -- right ascension of detector at t0 [rad].
                         For a detector network, put these as np.arrays with 
      ...
      self.detector_t0 = detector_t0
      self.detector_RA0 = detector_RA0
      self.detector_DEC0 = detector_DEC0
   ...
   def get_SNR_FisherMatrix(self, Ncores_mp=1):
      ...
      """
      # compute lifetime of source in band
      tD_in = (
         self.source_tc 
      ...
      tD_out = (
         self.source_tc 
         - waveform_LO.tau_fGW(self.source_Mc, self.detector_fGWmax)
         )
      ...
      #compute how many chunks of data the signal should be divided up into
      Nruns = int(
         np.ceil(
            (tD_out - tD_in)
      ...
      Ndetector = np.array(self.detector_RA0).size
      if np.array(self.detector_DEC0).size != Ndetector:
         print("detector_RA0 and detector_DEC0 have different length...")
         print("Aborting code")
         sys.exit()
      # if using multiprocessing, set the number of runs to a multiple of the number of cores/Ndetector
      if Ncores_mp > 1:
         Nruns = int(np.ceil(Nruns*Ndetector/Ncores_mp)*Ncores_mp/Ndetector)
      ...
      self.clock_r = antennaFuns_ground.orbit_earth_sun(np.array(self.clock_tD))
      # make list of run_params
      run_params_list = []
      if np.array(self.detector_RA0).ndim == 0:
      ...
      print(str(Nruns*Ndetector),
         "signal chunks will be computed...")
      if Ncores_mp > 1:
         ...
         for i in range(len(run_params_list)):
            ...
            print(
               ...
               "signal chunks computed; runtime:",
               str(int((time.time() - starttime)/60)),
               "min"
               )
      # collect results
      self.SNR2 = 0.
      self.FisherMat = np.zeros((8,8))
      ...
      source_params_dimful[4] = self.source_dL
      for i in range(self.FisherMat.shape[0]):
   ...
   def get_angular_resolution(self):
      ...
      self.angular_resolution = (
         ...
         * np.sqrt(
            self.CoVaMat[5,5]*self.CoVaMat[6,6]
            - self.CoVaMat[5,6]**2
            )
         )
      return
   def get_angular_resolution_priors(self):
      ...
      self.angular_resolution_priors = (
         ...
         * np.sin(np.pi/2. - self.source_DEC)
         * np.sqrt(
            self.CoVaMat_priors[5,5]*self.CoVaMat_priors[6,6]
            - self.CoVaMat_priors[5,6]**2
            )
         )
      return
